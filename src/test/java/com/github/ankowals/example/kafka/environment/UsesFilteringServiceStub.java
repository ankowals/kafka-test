...
public interface UsesFilteringServiceStub {
  ...
  WireMockStub FILTERING_SERVICE_STUB_INSTANCE =
      WireMockStub.start(
          ConfigureMock.filteringService().excludedValues(List.of("Zonk", "Kwak", "Hop")));

  default WireMockServer getFilteringServiceStub() {
    return FILTERING_SERVICE_STUB_INSTANCE.getServer();
  }

  default Map<String, String> getFilteringServiceProperties() {
    Map<String, String> properties = new HashMap<>();
    properties.put(
        "micronaut.http.services.filtering-service.url",
        FILTERING_SERVICE_STUB_INSTANCE.getServer().baseUrl());
