...
public interface UsesKafka {
  ...
  default AdminClient getAdminClient() {
    return KAFKA_INSTANCE.getAdminClient();
  }
  ...
  default Map<String, String> getKafkaProperties() {
    ...
    properties.put(
        "kafka.schema.registry.url", KAFKA_INSTANCE.getContainer().getSchemaRegistryUrl());
    properties.put(
        "kafka.streams.default.internal.leave.group.on.close",
        "true"); // to avoid re-balancing because of frequent app restarts in tests
