...
class StaticTopicTest extends IntegrationTestBase {
  ...
  @Inject StaticTopicTestProducer testProducer;

  @Inject ListBufferedStaticTopicTestConsumer testConsumer;
  ...
  void shouldConsumeProducedRecords() {
    String randomAlphabetic = RandomStringUtils.insecure().nextAlphabetic(11);

    this.testProducer.produce(randomAlphabetic);

    Awaitility.await()
        .untilAsserted(
            () -> {
              List<String> records = this.testConsumer.getRecords();
              Assertions.assertThat(records).contains(randomAlphabetic);
            });
  }
}
