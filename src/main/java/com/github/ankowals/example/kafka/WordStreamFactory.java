...
public class WordStreamFactory {
  ...
  @Singleton
  KStream<String, String> wordFilteringStream(
      ConfiguredStreamBuilder builder, FilteringServiceApiClient client) {
    Properties props = builder.getConfiguration();
    props.put(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, this.bootstrapServers);

    KStream<String, String> source =
        builder.stream("word-input", Consumed.with(Serdes.String(), Serdes.String()));

    source
        .peek((key, value) -> System.out.println("Incoming record: " + value))
